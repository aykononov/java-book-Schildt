...
>Обратите внимание на то, что закрытый метод getElements() вызывается в обоих методах popNElements() и skipAndPopNElements()
 с целью получить возвращаемый массив извлекаемых из стека элементов. Благодаря этому исключается дублирование одного и того
...
>[Chapter09/PrivateMethod/IntStack - версия интерфейса IntStack с закрытым методом](https://github.com/aykononov/JavaSchildt/blob/master/Chapter09/PrivateMethod/IntStack.java "Посмотреть пример")    
></details>
...
><details><summary>Общая форма блока обработки исключений</summary>
>
>```Java
>try {
>    // блок кода, в котором отслеживаются ошибки
>catch (тип_исключения_l ехOЬ) {
...
В Классе могут сохраняться данные о стояния, особенно с помощью переменных  экземпляра, тогда как в Интерфейсе этого сделать нельзя.  
